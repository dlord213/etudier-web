...
export const AddQuizComponent = ({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-4 p-4 dark:bg-stone-800 rounded-md">
        <input
          type="text"
          placeholder="Put your question here..."
          value={question}
          onChange={(e) => setQuestion(e.target.value)}
          className="outline-none lg:text-xl dark:bg-stone-800"
          maxLength={150}
        />
      ...
      {quiz.map((q, idx) => (
        <div
          key={idx}
          className="flex flex-col gap-2 p-4 dark:bg-stone-800 rounded-md"
        >
...
export const GenerateQuizComponent = ({
  ...
  const handleGenerate = async () => {
    ...
    const predefinedPrompt = `I wanna generate a quiz and the generated data must be formatted into something like this.
  {title: "", description:"", quizzes:[{userAnswers: ["", "", "", ""], question: "", correct_answer: number index in userAnswers}]}.
  Don't return any text, simply return an array with the object format I gave.
  If the prompt isn't related to generating quiz or subjects then return an appropriate result that's formatted like this {error: put an appropriate string here saying that the prompt isn't valid or isn't related on generating quiz.}.
  ...
  if (isGenerating === 1)
    return (
      <div className="flex flex-col items-center justify-center gap-6 lg:min-h-[75vh] min-h-[50vh] dark:bg-[#161616]">
  ...
  if (isGenerating === 3)
    return (
      <div className="flex flex-col items-center justify-center gap-4 dark:bg-[#161616]">
        <div className="flex flex-col gap-4">
          <MdClose
            size={28}
            className="cursor-pointer transition-all duration-200 hover:dark:text-stone-100 hover:dark:bg-stone-800 rounded-md"
